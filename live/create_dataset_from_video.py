...
def save_img_and_coordiantes(img, boxes_data, output_dir_img, output_dir_txt, frame_index):
...
while True:
    ...
    results = model(img, conf=0.7, device='cuda')
    ...
    for r in results:
        ...
        for box in boxes:
            ...
            cv2.putText(annotated_img, f"{classNames[cls]} {confidence}", (x1, y1-10),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 0), 2)
            print(f"Class: {classNames[cls]}, Confidence: {confidence}")

    if boxes_data:
        frame_counter += 1
        save_img_and_coordiantes(unannotated_image, boxes_data, work_dir_img, work_dir_txt, frame_counter)
